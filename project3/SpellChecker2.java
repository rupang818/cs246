...
public class SpellChecker2 {
    ...
    public double calculateLM(String word) {
        ...
        //
        // P(w) = Word count / total words count
        // Note: we get the same result even when we use dict.size(), but this is the right approach
        return dict.get(word)/(double)totalWordCount;
    }

    /*
        calculateEM: returns the error model P(correction|word)
    ...
    public double calculateEM(String word, String correction, double editDistance) {
        // Task4: 78.49% accuracy, 5.66% unknown
        System.out.println("Word: " + word + ", Correction: " + correction + ", EditDist: " + editDistance);
        // If the first characters are the same, more likely to be a match
        double gradScoring = graduatedScoring(word, correction, 0.4);
        // int divIndex = divergentIndex(word, correction);

        // If the word occurence is an outlier, don't consider them as candidates
        if (dict.get(correction) > 3500) {
            return 0.0;
        }

        // a,e,o,i,u: substitutions between vowels is more likely
        if (word.length() == correction.length()) {
            if (editDistance == 1.0 && gradScoring < 1.0) {     
                for (int i = 0; i < word.length(); ++i) {
                    if (word.charAt(i) != correction.charAt(i)) {
                        if (-1 < vowels.indexOf(word.charAt(i)) && -1 < vowels.indexOf(correction.charAt(i))) {
                            // vowel <-> vowel
        ...
        double toReturn = 0.0;
        if (editDistance == 1.0) {
            toReturn = Math.min(gradScoring + 0.2, 1.0);
            System.out.println("Min:" + toReturn);
            return toReturn;
        } else {
            if (word.length() == correction.length()) {
                toReturn = Math.max(gradScoring / 1.5, 0.025);
            } else {
                toReturn = Math.max(gradScoring / 15.0, 0.025);
            }
    ...
    // Returns the index of the divergence point index
    public static int divergentIndex(String word, String correction) {
        int i = 0;
        for(int j = 0; j < Math.min(word.length(), correction.length()); ++j) {
            if (word.charAt(j) != correction.charAt(j)) {
                break;
            }
            i++;
        }
    ...
    // Recursive function that calculates score based on how many letters from the beginning match
    // Assume they are of the same length
    public static double graduatedScoring(String word, String correction, double weight) {
        // System.out.println("Word: " + word + ", correction: " + correction + ", weight: " + weight);
        if (0 < word.length() && 0 < correction.length())  {
            if (word.charAt(0) == correction.charAt(0)) {
                return weight + graduatedScoring(word.substring(1), correction.substring(1), weight * 2.0);
            } else {
                return 0.0;
            }
        }
    ...
    private Map < String, Double > EditDist1(String word) {
        ...
        for (int i = 0; i <= word.length(); ++i) {
            ...
            if (L.length() > 0 && R.length() > 0) {
                String tempL = L.substring(0, L.length() - 1) + R.substring(0,1);
                String tempR = L.substring(L.length() - 1) + R.substring(1, R.length());
                temp = tempL + tempR;
